...
let nodes = [];
let links = [];
let simulationReady = false; // Flag to check if the simulation is ready
...
function createNewGraph() {
    ...
    alert(`New graph created with ID: ${uniqueId}`);
    restart()
    // Optionally, store the graph ID on the server or localStorage for sharing
}

// Function to upload a custom graph
function uploadGraph() {
    if (simulationReady) {
        console.log("clicked upload graph");
        document.getElementById('fileInput').click();
    } else {
        alert("Please wait a few seconds for the simulation to initialize.");
    }
}
